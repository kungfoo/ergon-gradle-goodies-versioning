 ...
 * <li>Supports specifying the abbrev length use (instead of just 7)</li>
 * </ul>
 */
public class BetterDescribeCommand extends GitCommand<String> {
	...
	/**
	 * How many tags we'll consider as candidates. This can only go up to the number of flags JGit can support in a
	 * walk, which is 24.
	 */
	...
	/**
	 * How long should abbreviated hashes be.
	 */
	private int abbreviate = 7;
	 ...
	 */
	protected BetterDescribeCommand(Repository repo) {
		super(repo);
		w = new RevWalk(repo);
	 ...
	 */
	public BetterDescribeCommand setTarget(ObjectId target) throws IOException {
	 ...
	 * Sets the commit to be described.
	 *
	 * @param rev Commit ID, tag, branch, ref, etc. See {@link org.eclipse.jgit.lib.Repository#resolve(String)} for
	 *            allowed syntax.
	 * @return {@code this}
	 * @throws IncorrectObjectTypeException the supplied id is not a commit or an annotated tag.
	 ...
	 * @throws java.io.IOException a pack file or loose object could not be read.
	 */
	public BetterDescribeCommand setTarget(String rev) throws IOException, RefNotFoundException {
	...
	/**
	 * Determine whether always to use the long format or not. When set to <code>true</code> the long format is used
	 * even the commit matches a tag.
	 *
	 * @param longDesc <code>true</code> if always the long format should be used.
	 * @return {@code this}
	 * @see <a href="https://www.kernel.org/pub/software/scm/git/docs/git-describe.html" >Git documentation about
	 *      describe</a>
	 * @since 4.0
	 */
	public BetterDescribeCommand setLongDesc(boolean longDesc) {
	...
	public BetterDescribeCommand setAbbreviate(int abbreviate) {
		this.abbreviate = abbreviate;
		return this;
	}

	/**
	 * Instead of using only the annotated tags, use any tag found in refs/tags namespace. This option enables matching
	 * lightweight (non-annotated) tags or not.
	 *
	 * @param tags <code>true</code> enables matching lightweight (non-annotated) tags like setting option --tags in c
	 *            git
	 * @return {@code this}
	 * @since 5.0
	 */
	public BetterDescribeCommand setTags(boolean tags) {
		this.useTags = tags;
		return this;
	}

	/**
	 * Always describe the commit by eventually falling back to a uniquely abbreviated commit hash if no other name
	 * matches.
	 *
	 * @param always <code>true</code> enables falling back to a uniquely abbreviated commit hash
	 * @return {@code this}
	 * @since 5.4
	 */
	public BetterDescribeCommand setAlways(boolean always) {
		this.always = always;
		return this;
	}
	...
	private String longDescription(Ref tag, int depth, ObjectId tip) throws IOException {
		return String.format("%s-%d-g%s", tag.getName().substring(R_TAGS.length()), //$NON-NLS-1$
			depth, w.getObjectReader().abbreviate(tip, abbreviate).name());
	}

	/**
	 * Sets one or more {@code glob(7)} patterns that tags must match to be considered. If multiple patterns are
	 * provided, tags only need match one of them.
	 *
	 ...
	 * @since 4.9
	 */
	public BetterDescribeCommand setMatch(String... patterns) throws InvalidPatternException {
		...
		if (tags == null || tags.isEmpty()) {
			...
			for (FileNameMatcher matcher : matchers) {
				Stream<Ref> m = tags.stream().filter(tag -> {
					matcher.append(tag.getName().substring(R_TAGS.length()));
			...
			return matchingTags.min(TAG_TIE_BREAKER);
		}
	}
	...
	public String call() throws GitAPIException {
		try {
			...
			Collection<Ref> tagList = repo.getRefDatabase().getRefsByPrefix(R_TAGS);
	 ...
	 *
	 * @param ref reference under inspection
	 * @return true if it should be used for describe or not regarding {@link BetterDescribeCommand#useTags}
	 */
