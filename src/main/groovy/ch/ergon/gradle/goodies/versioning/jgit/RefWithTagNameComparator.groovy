...
/**
 * Mimic tags comparator used by native git when doing `git describe --tags` and commit have multiple tags.
 *
 * Properties:
...
class RefWithTagNameComparator implements Comparator<RefWithTagName> {
    ...
    int compare(RefWithTagName tag1, RefWithTagName tag2) {
        boolean isTag1Annotated = isAnnotatedTag(tag1.ref)
        boolean isTag2Annotated = isAnnotatedTag(tag2.ref)
        ...
        if (!isTag1Annotated && !isTag2Annotated) {
            return tag1.ref.getName() <=> tag2.ref.getName()
        }

        // Both tags are annotated, try to return most recent one
        Date timeTag1 = getAnnotatedTagDate(tag1.ref)
        Date timeTag2 = getAnnotatedTagDate(tag2.ref)
        if (timeTag1 != null && timeTag2 != null) {
            // Smaller date means greater tag
            return timeTag2 <=> timeTag1
        }
